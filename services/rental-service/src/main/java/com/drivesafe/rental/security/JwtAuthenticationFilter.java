...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    public JwtAuthenticationFilter(@Value("${app.jwt.secret}") String secret) { this.key = Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8)); }
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if (header != null && header.startsWith("Bearer ")) {
            try {
                ...
                auth.setDetails(new RequestUser(((Number) claims.get("agencyId")).longValue(), ((Number) claims.get("userId")).longValue(), claims.getSubject(), role));
